...
const SITE_LINK: &str = "https://dustloop.com/wiki/api.php?action=cargoquery&format=json&limit=100&tables=MoveData_GBVSR&fields=MoveData_GBVSR.input%2C%20MoveData_GBVSR.name%2C%20MoveData_GBVSR.images%2C%20MoveData_GBVSR.hitboxes&where=chara%3D%22";
const SITE_HALF: &str =
...
pub async fn get_char_data(chars_ids: [&str; CHARS.len()], specific_char: &str) {
    ...
    if specific_char == "all" {
        for (x, char_id) in chars_ids.iter().enumerate() {
            ...
            char_json_schema = "\n]";
            write!(file, "{}", char_json_schema).expect(
                &("\nFailed to write 'json_schema' to ".to_owned()
                    + chars_ids[x]
                    + " 'images.json'."),
            );
        }
